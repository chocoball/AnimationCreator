    ...
    : QMainWindow(parent)
{
    ...
    m_pMdiArea = new DropableMdiArea;
...
void MainWindow::createActions(void)
{
    ...
    // オブジェクトスケール
    m_pActObjectScale = new QAction(trUtf8("オブジェクトスケール"), this);
    connect(m_pActObjectScale, SIGNAL(triggered()), this, SLOT(slot_objScale()));

    // フレームスケール
    m_pActFrameDataScale = new QAction(trUtf8("フレームスケール"), this);
    connect(m_pActFrameDataScale, SIGNAL(triggered()), this, SLOT(slot_frameScale()));
...
void MainWindow::createMenus(void)
{
    ...
    pMenu->addAction(m_pActFrameDataScale);
#if 0
...
bool MainWindow::fileOpen(QString fileName, bool bAdd)
{
    ...
    if (fileName.toLower().indexOf(".png") <= 0 && fileName.toLower().indexOf(".bmp") <= 0 && fileName.toLower().indexOf(".jpg") <= 0 && fileName.indexOf(FILE_EXT_ANM2D_XML) <= 0 && fileName.indexOf(FILE_EXT_ANM2D_BIN) <= 0)
    {
...
void MainWindow::checkFileModified(void)
{
    for (int i = 0; i < m_EditData.getImageDataListSize(); i++)
    {
        ...
        if (info.lastModified().toUTC() <= p->lastModified)
        {
            continue;
        }
